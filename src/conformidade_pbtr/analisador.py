...
from .modelos import Categoria, Documento, Relatorio, Resumo, Severidade, Status
...
def _calcular_resumo(rel: Relatorio) -> Resumo:
    ...
    for a in rel.achados:
        ...
        if a.categoria in (Categoria.TABELA, Categoria.VALOR) and a.status == Status.NAO_CONFORME:
            r.erros_tabela += 1
        if a.categoria == Categoria.ORTOGRAFIA and a.status == Status.NAO_CONFORME:
...
def analisar(
    ...
    """Executa a análise completa e devolve o Relatorio."""
    ...
    achados_ort, avisos = ortografia.validar(doc, usar_languagetool, limite_ortografia)
    rel.achados.extend(achados_ort)
